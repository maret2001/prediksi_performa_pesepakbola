...
def load_model():
  model_terbaik = "D:/My Drive/Semester 8/tugas_akhir/liga champions/skripsi/svm_model.pkl"
  with open(model_terbaik, 'rb') as file2:  
    model_svm = pickle.load(file2)
  return model_svm
...
def main():
  ...
  try:
    grid_table = AgGrid(data,
                        height = 500,
                        theme='balham') # --> menampilkan datasetnya
    ...
    print(e)
  # upload_data()
  tab_pemrosesan, tab_prediksi = st.tabs(['Pemrosesan Data','Prediksi'])
  # st.markdown("---")
  with tab_pemrosesan:
    pemrosesan()
  # st.markdown("---")
...
def pemrosesan():
  ...
  st.markdown("---")
  st.subheader("🔄️ Modelling SVM Classifier")
  ...
  if hyperpar_btn == "Ya":
    ...
    with col_gabungan:
      C = st.select_slider("Nilai C :", ("0.1","1","10","100","1000"))
      if C =="0.1":
        C = 0.1
      elif C =="1":
        C = 1
        ...
        C = 1000
      gamma = st.select_slider("Nilai gamma :", ('1','0.1','0.01','0.001','0.0001'))
      if gamma == '1':
        gamma = 1
      elif gamma == '0.1':
        gamma = 0.1
    ...
    with col_kernel:
      kernel = st.selectbox("Pilih Kernel :", ("rbf","linear","sigmoid","poly"))
    ...
    #   # function fitting dataset SVM Classifier
    #   def fit(self, X, y):
    #     self.m, self.n = X.shape # --> m = jumlah baris data, n = jumlah input features/kolom data
    #     self.y = np.where(y <= 0,1,2)
    ...
    if st.checkbox("Lihat Evaluasi"):
      ...
      pred_model = model_svm.predict(X_test_scaled)
      acc = model_svm.score(X_test_scaled, y_test)
      error = 1 - acc
      prec = precision_score(pred_model, y_test, average='macro')
      rec = recall_score(pred_model, y_test, average='macro')
      f1_s = f1_score(pred_model, y_test, average='macro')
...
def prediksi():
  ...
  # daftar widgetnya
  # posisi = st.sidebar.number_input("**Position :**",0,3,1)
  ...
  if st.sidebar.button("**Lihat Prediksi**"):
    # 1. Preprocess
    # data_baru = [[posisi,pass_acc, pass_comp, cross_acc, cross_comp, 
    #               freekicks_taken, match_played, jumlah_gol,jumlah_asis, dis_covered]]
    ...
    # 2. prediksi
    hasil_prediksi = model_svm.predict(fitur)
    hasil_prediksi = int(hasil_prediksi)
    # 3. tampilan outputnya
